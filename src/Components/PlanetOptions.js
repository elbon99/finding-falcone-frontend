...
function PlanetOption(props) {
    ...
    return (
        <div className="select-destination">
            ...
            <select autoComplete='on' defaultValue="" value={selectPlanet} onChange={(event) => handlePlanetChange(event,selectPlanet)}>
                    ...
                    {props.vehicles.map(vehicle => {
                        return (
                            <div className="inner-vehicles" key={vehicle.name}>
                                <input type="radio" id={vehicle.name} name={props.index} value={vehicle.name} defaultChecked={vehicle.name === selectVehicle}
